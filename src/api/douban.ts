 ...
 */
export async function getWatched(userId: string, pageCount: number = 1): Promise<MovieItem[]> {
  const pageSize = 15;
  const tasks = [];

  for (let page = 0; page < pageCount; page++) {
    const url = `https://movie.douban.com/people/${userId}/collect?sort=time&start=${page * pageSize}&mode=grid&tags_sort=count`;
    ...
    tasks.push(
      ...
      .then(html => {
        ...
        $('.grid-view .item').each((_, element) => {
          ...
          const title = $info.find('li.title a').text().trim().split('/')[0].trim();
          const link = $info.find('li.title a').attr('href') || '';
          const cover = formatCoverUrl($item.find('.pic img').attr('src') || '');
